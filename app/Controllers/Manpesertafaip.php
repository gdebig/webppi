...
class Manpesertafaip     extends BaseController
{
    public function index()
    {
        ...
        $where1 = "status IN ('lulus')";
        $user = $model->join('tbl_profile', 'tbl_user.user_id = tbl_profile.user_id', 'left')->where($where1)->where($where)->where('tbl_user.softdelete', 'no')->orderby('tbl_user.user_id', 'DESC')->findall();
        if (!empty($user)) {
            $data['data_user'] = $user;
        } else {
        ...
        return view('maintemp/pesertafaip', $data);
    }
